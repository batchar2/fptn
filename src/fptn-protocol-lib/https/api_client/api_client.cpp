...
Response ApiClient::Get(const std::string& handle, int timeout) {
  ...
  try {
    ...
    auto socket = CreateObfuscatedSocket(ioc);

    boost::beast::net::ip::tcp::resolver resolver(ioc);

    // Используем SocketSPtr напрямую в ssl_stream
    boost::beast::ssl_stream<obfuscator::Socket&> stream(*socket, ctx);
...
fptn::protocol::https::obfuscator::SocketSPtr ApiClient::CreateObfuscatedSocket(
...
// #include "https/obfuscator/socket/socket.h"
//
// #ifdef _WIN32
// #pragma warning(pop)
// #endif
...
//   return {{"Host", host},
//       {"sec-ch-ua",
//           R"("Chromium";v="128", "Not;A=Brand";v="24", "Google
//           Chrome";v="128")"},
//       {"sec-ch-ua-platform", "\"macOS\""}, {"sec-ch-ua-mobile", "?0"},
//       {"upgrade-insecure-requests", "1"},
//       {"User-Agent",
...
//       {"Referer", "https://www.google.com/"},
//       {"Accept-Encoding", "gzip, deflate, br, zstd"},
//       {"Accept-Language", "en-US,en;q=0.9,ru;q=0.8"}, {"priority", "u=0,
//       i"}};
// #else
// #error "Unsupported system!"
// #endif
...
//
// ApiClient::ApiClient(
//     const std::string& host, int port, obfuscator::IObfuscatorSPtr
//     obfuscator) : host_(host),
//       port_(port),
//       sni_(host),
//       obfuscator_(std::move(obfuscator)) {}
//
...
//     boost::beast::net::ip::tcp::resolver resolver(ioc);
//
//     boost::beast::ssl_stream<obfuscator::SocketSPtr> stream(
//         std::move(socket), ctx);
//
//     const std::string port = std::to_string(port_);
//     auto const results = resolver.resolve(host_, port);
//
//     stream.lowest_layer().expires_after(std::chrono::seconds(timeout));
...
//
//     fptn::protocol::https::utils::SetHandshakeSessionID(stream.native_handle());
//     fptn::protocol::https::utils::SetHandshakeSni(stream.native_handle(),
//     sni_);
//
//     if (!expected_md5_fingerprint_.empty()) {
//       ssl = stream.native_handle();
...
// }
//
// fptn::protocol::https::obfuscator::SocketSPtr
// ApiClient::CreateObfuscatedSocket(
//     boost::asio::io_context& ioc) const {
//   auto socket =
//       std::make_shared<obfuscator::Socket>(ioc.get_executor(), obfuscator_);
//   return socket;
